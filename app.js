...
function lineChart(points){if(points.length<2)return "";const W=520,H=190,P=30,ts=points.map(x=>new Date(x.date).getTime()),vs=points.map(x=>+x.temp),mn=Math.min(...vs)-.5,mx=Math.max(...vs)+.5,t0=Math.min(...ts),t1=Math.max(...ts)||t0+1;const xy=points.map(x=>{const px=P+(new Date(x.date).getTime()-t0)/(t1-t0)*(W-P*2),py=H-P-(+x.temp-mn)/(mx-mn)*(H-P*2);return [px,py]}),poly=xy.map(p=>p.join(",")).join(" ");return '<svg class="temp-chart" viewBox="0 0 '+W+' '+H+'" role="img" aria-label="水温推移"><line x1="'+P+'" y1="'+(H-P)+'" x2="'+(W-P)+'" y2="'+(H-P)+'"/><line x1="'+P+'" y1="'+P+'" x2="'+P+'" y2="'+(H-P)+'"/><polyline points="'+poly+'"/>'+xy.map((p,i)=>'<circle cx="'+p[0]+'" cy="'+p[1]+'" r="4"><title>'+points[i].date+' '+Number(points[i].temp).toFixed(1)+'℃</title></circle>').join("")+'<text x="4" y="'+(P+4)+'">'+mx.toFixed(1)+'℃</text><text x="4" y="'+(H-P)+'">'+mn.toFixed(1)+'℃</text></svg>'}
function historyPanel(){const groups=[["蔣淵・大島 1m","komobuchi"],["宿毛湾","sukumo"]];return '<div class="history-wrap">'+groups.map(([name,id])=>{const a=historyData.stations?.[id]||[];if(!a.length)return "";const chart=lineChart(a.slice(-30));const rows=a.slice(-8).map(v=>'<tr><td>'+new Date(v.date).toLocaleDateString("ja-JP")+'</td><td>'+(v.depthM??"—")+'m</td><td>'+Number(v.temp).toFixed(1)+'℃</td><td>'+v.kind+'</td></tr>').join("");return '<article class="card"><h3>'+name+'</h3>'+chart+'<table class="history-table"><thead><tr><th>日付</th><th>水深</th><th>水温</th><th>種別</th></tr></thead><tbody>'+rows+'</tbody></table></article>'}).join("")+'</div>'}
