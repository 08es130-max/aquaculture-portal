...
function historyPanel(){const groups=[["蔣淵・大島 1m","komobuchi"],["宿毛湾","sukumo"]];return '<div class="history-wrap">'+groups.map(([name,id])=>{const a=historyData.stations?.[id]||[];if(!a.length)return "";const rows=a.slice(-8).map(v=>'<tr><td>'+new Date(v.date).toLocaleDateString("ja-JP")+'</td><td>'+(v.depthM??"—")+'m</td><td>'+Number(v.temp).toFixed(1)+'℃</td><td>'+v.kind+'</td></tr>').join("");return '<article class="card"><h3>'+name+'</h3><table class="history-table"><thead><tr><th>日付</th><th>水深</th><th>水温</th><th>種別</th></tr></thead><tbody>'+rows+'</tbody></table></article>'}).join("")+'</div>'}
