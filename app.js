...
document.getElementById("app").innerHTML=h}function setTab(t){active=t;render();scrollTo({top:0,behavior:"smooth"})}function filterNews(a,b){document.querySelectorAll(".filters button").forEach(x=>x.classList.remove("active"));b.classList.add("active");document.querySelectorAll("#news [data-area]").forEach(x=>x.style.display=(a==="すべて"||x.dataset.area===a)?"block":"none")}async function loadSeaData(){try{const r=await fetch("data/sea-temperature.json?"+Date.now());if(!r.ok)return;const d=await r.json();for(const s of d.stations||[]){const x=sea.find(v=>v.area===s.area);if(!x)continue;if(s.temp!==null&&s.temp!==undefined)x.temp=Number(s.temp).toFixed(1)+"℃";if(s.observedAt)x.at=s.observedAt;if(s.status)x.status=s.status;if(s.source)x.url=s.source;if(s.note)x.note=s.note;if(s.depths)x.depths=s.depths;if(s.change1d!=null)x.change1d=s.change1d;if(s.change7d!=null)x.change7d=s.change7d;}document.getElementById("updated").textContent="v0.3 ・ データ更新 "+new Date(d.updatedAt).toLocaleString("ja-JP");render()}catch(e){console.warn("data load failed",e)}}document.getElementById("updated").textContent="v0.3 ・ 2026/9/22";render();loadSeaData();